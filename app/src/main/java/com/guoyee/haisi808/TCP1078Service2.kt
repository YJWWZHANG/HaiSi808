 ...
 *创建时间:2019/1/9 16:24
 */
class TCP1078Service2: Service() {

    private val mTcpVideoPort: Int = 5001
        ...
        override fun run() {
            ...
            while (!isInterrupted) {
                try {
                    ...
                    LogUtils.e("TCP1078_2视频已经成功连接")
                    ToastUtils.showLong("TCP1078_2视频已经成功连接")
                    Thread(Runnable {
                        try {
...
//        }
        ...
        override fun run() {
            super.run()
            LogUtils.e("TCP1078_2视频解析线程已开启")
            while (true) {
                if (mVideoQueue.size > 0) {
//                    LogUtils.e("TCP1078_2开始")
...
//                        LogUtils.e("$binaryString $tag")
                        when(tag) {
                            0.toByte() -> {     //原子包
                                ...
                                EventBus.getDefault().post(ShowVideo2Event(Bytes.toArray(mDataBufferList)))
